...
variable "vpc_id" {
  description = "Tokyo AWS VPC ID (빈 문자열이면 새로 생성)"
  type        = string
  default     = ""
}

variable "vpc_cidr" {
  description = "Tokyo AWS VPC CIDR (예: 10.0.0.0/16)"
  type        = string
  default     = "10.0.0.0/16"
}
...
variable "public_subnet_ids" {
  description = "ELB용 Public Subnet IDs (기존 VPC 사용 시 필수)"
  type        = list(string)
  default     = []
}
...
variable "cognito_domain_prefix" {
  description = "Cognito Hosted UI 도메인 prefix (전 계정 내 유니크, 예: corp-portal-tokyo)"
  type        = string
}
...
variable "cors_allowed_origins" {
  description = "API Gateway CORS에 허용할 Origin 목록"
  type        = list(string)
  default     = [
    "http://localhost:3000"
  ]
...
variable "existing_rds_instance_identifier" {
  description = "기존 Aurora 인스턴스 식별자"
  type        = string
  default     = "aurora-global-seoul-writer"
}
